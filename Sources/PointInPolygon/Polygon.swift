...
public class Polygon {
    ...
    /**
     Initializes a polygon from `Point` objects, as well as `Polygon`
     objects which are holes in the parent polygon.
     
     - note: Multiple hole polygons can be nested within hole polygons,
     or they can all be added in a list to the parent polygon; either
     case will not impact the result of the `Polygon.contains(point:)`
     method. If a polygon's holes overlap, the intersection area of the
     hole polygons will be counted as falling within the polygon.
    ...
    /**
     ...
     - returns: A case of `Polygon.ContainsPointResult` which describes
